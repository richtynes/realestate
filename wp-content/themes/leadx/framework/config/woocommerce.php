...
// Disable WooCommerce CSS
	...
	if(!empty(get_theme_mod('leadx_wc_items', '12'))){
		add_filter( 'loop_shop_per_page', create_function( '$cols', 'return ' . get_theme_mod('leadx_wc_items', '12') . ';' ), 20 );
	}

	// Toggle Sort by Function
	if(get_theme_mod('leadx_wc_shop_sort', 'yes') == 'no'){
		remove_action( 'woocommerce_before_shop_loop', 'woocommerce_catalog_ordering', 30 );
	}

	// Toggle Result Count
	if(get_theme_mod('leadx_wc_result_count', 'yes') == 'no'){
		remove_action( 'woocommerce_before_shop_loop', 'woocommerce_result_count', 20 );
	}

	// Toggle Upsell Products
	if(get_theme_mod('leadx_wc_upsell', 'no') != 'yes'){
		remove_action( 'woocommerce_after_single_product_summary', 'woocommerce_upsell_display', 15);
	}

	// Toggle Related Products
	if(get_theme_mod('leadx_wc_related', 'yes') == 'no'){
		remove_action( 'woocommerce_after_single_product_summary', 'woocommerce_output_related_products', 20);
	}

	// Toggle Add to Cart Button
	if(get_theme_mod('leadx_wc_add_to_cart', 'no') != 'yes'){
	...
	function leadx_woo_remove_styles() {
		wp_dequeue_style( 'woocommerce_prettyPhoto_css' );
		wp_dequeue_script( 'select2' );
	}
	add_action( 'wp_enqueue_scripts', 'leadx_woo_remove_styles', 99 );
