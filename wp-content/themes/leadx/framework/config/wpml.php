...
if ( ! class_exists( 'LeadX_WPML_Config' ) ) {

	class LeadX_WPML_Config {
		...
		public function __construct() {
			...
			// Add Actions
			add_action( 'admin_init', array( $this, 'leadx_register_strings' ) );
			
			// Add dropdown to widgets
			add_action( 'in_widget_form', array( $this, 'leadx_widget_dropdown' ), 10, 3 );

			// Update dropdown value on widget update
			add_filter( 'widget_update_callback', array( $this, 'leadx_widget_update' ), 10, 4 );
	
			// Filter widgets by language
			add_filter( 'widget_display_callback', array( $this, 'leadx_display_widget' ), 10, 3 );

			// Add Filters
			add_filter( 'upload_dir', array( $this, 'leadx_convert_base_url' ) );
		}
		 ...
		 *
		 */
		public function leadx_register_strings() {
			if ( function_exists( 'icl_register_string' ) && $strings = leadx_register_theme_mod_strings() ) {
		 ...
		 *
		 */
		public function leadx_convert_base_url( $param ) {
		 ...
		 */
		public function leadx_widget_dropdown( $widget, $form, $instance ) {
	
			$languages = icl_get_languages();
	
			?><p>
				<label for='wpml_language'><?php esc_html_e( 'Display on language:', 'leadx' ); ?> </label>
				...
				?><option <?php selected( $selected ); ?> value='all'><?php esc_attr_e( 'All Languages', 'leadx' ); ?></option>
		 ...
		 */
		public function leadx_widget_update( $instance, $new_instance, $old_instance, $this2 ) {
		 ...
		 */
		public function leadx_display_widget( $instance, $widget, $args ) {
