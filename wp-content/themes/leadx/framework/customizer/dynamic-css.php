...
if ( class_exists( 'WP_Customize_Setting' ) && ! class_exists( 'LeadX_Customize_CSS' ) ) {
	class LeadX_Customize_CSS extends WP_Customize_Setting {
		/*  for PRO users! - *
		 ...
		 * @return 2D array
		 */
		public function leadx_get_css_map() {
			return $this->css_map;
		}
		 ...
		 * @return array
		 */
		public function leadx_get_all_css_properties() {
			return array_keys( $this->leadx_get_css_map() );
		}
		 ...
		 */
		public function leadx_css_selector_groups_for_property( $css_property ) {
			...
			if ( array_key_exists( $css_property , $this->css_map ) ) {
				...
				foreach ( $this->css_map[ $css_property ] as $css_selector ) {
					if ( self::leadx_is_filterable_string( $css_selector ) ) {
		 ...
		 */
		public function leadx_plain_selectors_for_all_groups( $css_property ) {
			$selectors = array();
			$all_groups = $this->leadx_css_selector_groups_for_property( $css_property );
		 ...
		 */
		public function leadx_setting_render_css() {
			$out = '';

			foreach ( $this->leadx_get_css_map() as $css_prop_raw => $css_selectors ) {
				// we get here the $css_prop and $value
				extract( $this->leadx_filter_css_property( $css_prop_raw ) );

				foreach ( $this->leadx_css_selector_groups_for_property( $css_prop_raw ) as $media_query => $css_selectors_arr ) {
		 ...
		 */
		public function leadx_filter_css_property( $css_property ) {
			// defaults
			$out = array(
			...
			if ( self::leadx_is_filterable_string( $css_property ) ) {
				...
				if ( preg_match( '/^([a-z0-9\_]+)(?:\((\w+)\))?$/i', trim( $filter ), $matches ) ) {
					switch ( $matches[1] ) {
						case 'darken':
							$out['value'] = $this->leadx_darken_css_color( $this->value(), (int) $matches[2] );
							break;
						case 'lighten':
							$out['value'] = $this->leadx_lighten_css_color( $this->value(), (int) $matches[2] );
							break;
						case 'important':
						...
						case 'linear_gradient_to_bottom':
							$out['value'] = sprintf( '%1$s linear-gradient(to bottom, %1$s, %2$s)', $this->value(), $this->leadx_darken_css_color( $this->value(), (int) $matches[2] ) );
							break;
						default:
		 ...
		 * @return boolean
		 */
		public static function leadx_is_filterable_string( $css_property ) {
		 ...
		 */
		protected static function leadx_darken_css_color( $color, $percent = 20 ) {
			$alter_for = 2.55 * $percent;

			$parts = self::leadx_rgb_from_hexdec( $color );
		 ...
		 * @return string Hexdec color.
		 */
		protected static function leadx_lighten_css_color( $color, $percent = 20 ) {
			$alter_for = 2.55 * $percent;
		 ...
		 * @return array
		 */
		protected static function leadx_rgb_from_hexdec( $hexdec ) {
			$rgb = array();
