...
$site_bg_color = get_post_meta($id, 'leadx_post_site_bg_color', true);
?>
<body <?php body_class(); ?> <?php if($site_bg_color != '') { echo 'style="background-color:' . esc_attr($site_bg_color) . ';"'; } ?>>

<?php
	$sticky_style = get_theme_mod( 'leadx_header_sticky', 'sticky' );
	$header_style = get_theme_mod( 'leadx_header_style', 'header-top-full');
...
?>
<div id="page" class="site-wrapper <?php echo get_theme_mod( 'leadx_site_layout', 'full-width' ) . ' ' . $header_style ?>">
    ...
    <?php
        ...
        if ( get_post_meta( $id, 'leadx_post_header', true ) != 'hide' ) {
        	//get page title style from page settings, global default is color
			$header_bg = get_post_meta( $id, 'leadx_post_header_background', true );
			if ( empty( $header_bg ) ) {
	            $header_bg = 'color';
	        }
	        
			if($header_bg == 'slider') {
	        	$slider = get_post_meta( $id, 'leadx_post_header_slider', true );
				echo do_shortcode($slider); 
			}
			else {
				//Set header height. Get from page settings or global customizer
				$header_height = get_post_meta( $id, 'leadx_post_header_height', true );
				if ( empty( $header_height ) ) {
		            $header_height = get_theme_mod('leadx_pagetitle_height', 'x-small');
		        }
